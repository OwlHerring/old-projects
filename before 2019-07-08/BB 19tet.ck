...
//
//
// melody (format: [note in chronological order][BBB?, t[#] of repDur, t[#] of totalDur, # of reps, key, accidental]:
[ [1,4,2,2,21,2], [1,4,2,1,25,2], [0,8,2,6,21,2]] @=> int mel[][];
...
while(true)
{
    ...
    if(mel[melCount%mel.size()][0])
    {
        spork ~ BBB( t[mel[melCount%mel.size()][1]], t[2], mel[melCount%mel.size()][3], gamut[ dia[ mel[melCount%mel.size()][4] ][ mel[melCount%mel.size()][5] ] ], t[mel[melCount%mel.size()][2]], dac );
    }
    else
    {
        spork ~ BB( t[mel[melCount%mel.size()][1]], t[2], mel[melCount%mel.size()][3], gamut[ dia[ mel[melCount%mel.size()][4] ][ mel[melCount%mel.size()][5] ] ], t[mel[melCount%mel.size()][2]], dac );
    }
    
    t[mel[melCount%mel.size()][2]] => now;
