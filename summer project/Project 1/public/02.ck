...
public class EDO 
{
    ...
    fun float C(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[0] + chromAlt[ini.modPlus(tonic()-0,letterSemitones.size(),0)] + accidental(acci + keySig[key][0])); }
    fun float C(int oct)          { return C(oct, 0); }
    fun float D(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[1] + chromAlt[ini.modPlus(tonic()-1,letterSemitones.size(),0)] + accidental(acci + keySig[key][1])); }
    fun float D(int oct)          { return D(oct, 0); }
    fun float E(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[2] + chromAlt[ini.modPlus(tonic()-2,letterSemitones.size(),0)] + accidental(acci + keySig[key][2])); }
    fun float E(int oct)          { return E(oct, 0); }
    fun float F(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[3] + chromAlt[ini.modPlus(tonic()-3,letterSemitones.size(),0)] + accidental(acci + keySig[key][3])); }
    fun float F(int oct)          { return F(oct, 0); }
    fun float G(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[4] + chromAlt[ini.modPlus(tonic()-4,letterSemitones.size(),0)] + accidental(acci + keySig[key][4])); }
    fun float G(int oct)          { return G(oct, 0); }
    fun float A(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[5] + chromAlt[ini.modPlus(tonic()-5,letterSemitones.size(),0)] + accidental(acci + keySig[key][5])); }
    fun float A(int oct)          { return A(oct, 0); }
    fun float B(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[6] + chromAlt[ini.modPlus(tonic()-6,letterSemitones.size(),0)] + accidental(acci + keySig[key][6])); }
