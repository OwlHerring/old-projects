public class Ini // fuck it, this'll be where any useful function goes.
{
    // update with other folders in the project.
    [
    me.dir() + "/public/",     // 0
    me.dir() + "/public/instr/",// 1
    me.dir() + "/main/"        // 2
    ]
    @=> static string S[];
...
for(0 => int addCount; addCount < 100; addCount++) // The ability to read how many files are in a folder doesn't seem to be in ChucK.
{
    if(!ini.add(0, addCount)) { break; }
}
for(0 => int addCount; addCount < 100; addCount++) // Then it does the instruments.
{
