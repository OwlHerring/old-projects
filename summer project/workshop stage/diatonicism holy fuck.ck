...
my.setChromAlt( [0, 0, -1, 0, 0, 0, 0] );

my.setKey(1); // D Major / B minor.

// leaving this debugging up right now for posterity, as a reminder to future generations of the cost of freedom
<<< my.tonic() >>>;
<<< my.letterSemitones.size() >>>;
<<<" ">>>;
for(0 => int count; count < my.chromAlt.size(); count++){ <<< my.chromAlt[count] >>>; }
<<<" ">>>;
for(0 => int count; count < my.chromAlt.size(); count++){ <<< my.chromAlt[modPlus(count-my.tonic(), my.letterSemitones.size(),0)] >>>; }
//<<< my.chromAlt[modPlus(2+my.tonic(), my.letterSemitones.size(),0)] >>>;

minute / 60 => dur beat;
3 => int oct;
[
//              E  2
createNote( my.dia(my.tonic()  , oct), 2::beat, 1 ),
createNote( my.dia(my.tonic()+1, oct), 2::beat, .8 ),
createNote( my.dia(my.tonic()+2, oct), 4::beat, .8 )
...
while(true)
{
    music[musicCount%music.size()].freq => s.freq;
    
    music[musicCount%music.size()].length => now;
