...
c.modFreq(5.8);
c.modDepth(0.007); // this gets a decent vibrato.
c.mix(.6); // 0: just the original; 1: just something going up and down the old; 0.5: half both.
...
p.setFadeRatio(1.5); // This is the ratio between the fading times of harmonic N and harmonic N+1. Be careful < .5!  
p.a(.3);             // This is the relative strength of harmonics. Closer to .001: ponticello. Closer to .99: sul tasto.
p.set(.3::second, .2::second, .5, .1::second); // sets ADSR of the noise being filtered.
p.allow();
p.hold(1);

110 => float freq;
[freq, freq*1.5, freq*2] @=> float freqs[];
0 => int freqsCount;
...
while(true)
{
    p.setBow(freqs[0]* 1.5 * bowRatio[bowCount%bowRatio.size()] );
    p.set(.25::second, freqs[freqsCount%freqs.size()]); // sets basically the ring of the string (plus the frequency). Too low and it'll be very airy no matter what. Too high and it'll always be laissez vibre
    p.hit(1);
