...
private class EDO 
{
    ...
    //  returns, in Hertz: (x octaves times EDO, plus distance between white keys, plus diatonic alterations) semitones above C0.
    fun float C(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[0] + accidental(acci + keySig[key][0])); }
    fun float C(int oct)          { return C(oct, 0); }
    fun float D(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[1] + accidental(acci + keySig[key][1])); }
    fun float D(int oct)          { return D(oct, 0); }
    fun float E(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[2] + accidental(acci + keySig[key][2])); }
    fun float E(int oct)          { return E(oct, 0); }
    fun float F(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[3] + accidental(acci + keySig[key][3])); }
    fun float F(int oct)          { return F(oct, 0); }
    fun float G(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[4] + accidental(acci + keySig[key][4])); }
    fun float G(int oct)          { return G(oct, 0); }
    fun float A(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[5] + accidental(acci + keySig[key][5])); }
    fun float A(int oct)          { return A(oct, 0); }
    fun float B(int oct, int acci){ return C0 * semi(tone*oct + letterSemitones[6] + accidental(acci + keySig[key][6])); }
    ...
    {
        //     C0 in Hz times the following semitones: [semitones per octave] times (the sum of [octaves above 0] and any extra octaves (from inputting 8 diatonic steps or something)), plus the semitone value of the inputted diatonic steps (mod diatonic scale size), plus accidental modifier based on key signature                                          and any additional accidental beyond that.
        return C0       *     semi(                    tone                   *     (oct                          +   modPlus(letter,letterSemitones.size(),1))                          +    letterSemitones[      modPlus(letter, letterSemitones.size(),0)]             +    accidental(                  keySig[key][modPlus(letter, letterSemitones.size(),0)] +  acci)                                );
    }
...
fun Note createNote(float freq, dur length)
{
    Note newNote;
    freq => newNote.freq;
...
fun Note createNote(float freq, dur length, float gain)
{
    Note newNote;
    freq => newNote.freq;
    length => newNote.length;
...
[
//              E  2
createNote( my.dia(9, oct), .75::beat, 1 ),
createNote( my.dia(8, oct), .25::beat, .8 ),
createNote( my.dia(9, oct),   1::beat, .8 ),
//              B 1
createNote( my.dia(6, oct),   1::beat, .7 ),

createNote( my.dia(7, oct), .75::beat, .9 ),
createNote( my.dia(6, oct), .25::beat, .8 ),
createNote( my.dia(7, oct),   1::beat, .7 ),
createNote( my.dia(4, oct),   1::beat, .6 ),

createNote( my.dia(5, oct), .75::beat, .8 ),
createNote( my.dia(4, oct), .25::beat, .7 ),
createNote( my.dia(5, oct),   1::beat, .7 ),
createNote( my.dia(3, oct),   1::beat, .6 ),

createNote( my.dia(6, oct),   2::beat, .6 ),
createNote( my.dia(2, oct),   1::beat, .5 )
...
fun void p1(PingB p, dur fadeLength)
{
    p => Envelope fade => safe;
    
    fade.value(0);
    fade.target(1);
    fade.duration(.05::second);
    
    p.hit(1);
    now + beat => time later;
    fade.duration() => now;
    fade.target(0);
    fade.duration(fadeLength);
    later => now;
    while(fade.value()>0)
    {
        p.hit(1);
...
while(true)
{
    maybe => int Maybe;
    maybe => int Maybe2;
    Math.random2f(1,10)::second => dur seconds;
    [Math.random2f(.8,1) ] @=> float pMod[];
    pMod << Math.pow(pMod[0],-1);
    
    Shred myP[0];
    myP << spork ~ p1(p[Maybe], pMod[Maybe2]::seconds);
    myP << spork ~ p1(p[!Maybe], pMod[!Maybe2]::seconds);
