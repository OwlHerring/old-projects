...
Event theBells;
spork ~ bellsBellsBells(1, .8, 7, 1);

35::beat => now;
theBells => now;

Math.random2f(.0,.03)::second => now;

spork ~ bellsBellsBells(.5, .6, 3, 0);
...
spork ~ bellsBellsBells(.5, .2, 6, 0);
Math.random2f(.0,.03)::second => now;
spork ~ bellsBellsBells(1, .5, 2, 0);

35::beat => now;
theBells => now;

Math.random2f(.0,.01)::second => now;

spork ~ bellsBellsBells(2, .1, 9, 0);
Math.random2f(.0,.01)::second => now;
spork ~ bellsBellsBells(2, .12, 7, 0);
Math.random2f(.0,.01)::second => now;
spork ~ bellsBellsBells(2, .11, 5, 0);

105::beat => now;

fadeout.target(0);
fadeout.duration(9::beat);

fadeout.duration()+2::beat => now;
...
fun void bellsBellsBells(float beatModifier, float A, int Add, int forWhom)
{
    ...
    while(true)
    {
        ...
        400 + 50*Math.random2(0+gradual,4+gradual) => float next;
