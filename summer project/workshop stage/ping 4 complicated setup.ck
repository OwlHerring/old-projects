...
//1000 => int rep;
55 => float f; second / f => dur fPeriod;
//(rep*Math.pow(f,-1))::second => dur shwipTime;
// rep::fPeriod is shwipTime.

.3::second => dur shwipTime;
Std.ftoi(freq*ratio*shwipTime/second) => int rep;

for(0 => int pCount; pCount < p.size() ; pCount++)
{ 
    new Ping3 @=> p[pCount]; 
    p[pCount].set(.2::second, 440);
    p[pCount].setShwip(rep, .95, Math.pow(rep,-1)::shwipTime);
